...
class EventViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if let name = eventList.first?.eventClass.trimmingCharacters(in: .whitespacesAndNewlines), !name.isEmpty {
            className = name
            eventFound.text = "\(eventList.count) events found in \(name)"
        } else {
            className = "Unnamed Class"
            eventFound.text = "\(eventList.count) events found"
        }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            if let name = eventList.first?.eventClass {
                className = name
                eventFound.text = "\(eventList.count) events found in \(name)"
            } else {
